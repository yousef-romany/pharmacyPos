...
import type { SaleTransaction, SaleTransactionItem, Customer, PaymentMethod, Product } from '@/lib/types'; // Import type and Customer
import { getSales, getCustomers, getProductById, getCustomerById, deleteSale, getProducts } from '@/lib/data'; // Import data fetching functions, add deleteSale
import {
   Dialog,
   DialogContent,
   DialogHeader,
...
const getPaymentMethodInfo = (method: PaymentMethod | undefined) => {
    if (!method) return { text: method || '', icon: Coins, color: 'text-muted-foreground' };
    switch (method) {
            ...
            try {
                const itemsWithDetails = await Promise.all(sale.items.map(async (item) => {
                     ...
                     const originalUnitPrice = product ? (item.soldUnitType === 'sub' && product.subUnitsPerUnit
                        ...
                        : safeParseFloat(product.price)) : safeParseFloat(item.price);
                 ...
                 <body>
                      ...
                      <table>
                       ...
                       <div class="totals">
                          ...
                          ${insuranceDiscountAmount > 0 ? `<div><span class="discount">خصم التأمين:</span> <span class="discount">- ${insuranceDiscountAmount.toFixed(2)} ر.س</span></div>` : ''}
              ...
              <div className="py-4 space-y-4">
                 <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                     ...
                     <p><strong className="ml-1">طريقة الدفع:</strong>
                          <Badge variant="outline" className={cn("mr-1 px-1.5 py-0.5 text-xs", paymentInfo.color)}>
...
function PrintInvoiceDialog({ sale, customer, onClose }: PrintInvoiceDialogProps) {
    ...
    React.useEffect(() => {
        const fetchProducts = async () => {
            ...
            try {
                ...
                setProducts(allProducts.filter(product => productIds.includes(product.id)));
            } catch (error) {
                console.error('Failed to fetch products for invoice:', error);
                toast({ title: "خطأ", description: "فشل تحميل بيانات المنتجات.", variant: "destructive" });
            } finally {
                    ...
                    {isLoadingProducts ? (
                        <div className="flex items-center justify-center py-12">
                            <div className="text-center">
                                ...
                                <p className="text-sm text-gray-600">جاري تحميل بيانات الفاتورة...</p>
                            </div>
                        </div>
                    ) : (
                        <InvoiceWrapper
...
export default function SalesPage() {
  ...
  const [customers, setCustomers] = React.useState<Map<string, Customer>>(new Map()); // Map customer ID to Customer object
  ...
  const [saleToPrint, setSaleToPrint] = React.useState<SaleTransaction | null>(null); // For print invoice dialog
  const { toast } = useToast();
  ...
  React.useEffect(() => {
   ...
   const filteredSales = sales.filter(sale =>
      ...
      (sale.customerId && customers.get(sale.customerId)?.name?.toLowerCase().includes(searchTerm.toLowerCase())) ||
   ...
   return (
      <>
                        ...
                        filteredSales.map((sale) => {
                           ...
                           return (
                              <TableRow key={sale.id}>
                                 ...
                                 <TableCell>{sale.customerId ? (customers.get(sale.customerId)?.name || sale.customerId) : 'عميل نقدي'}</TableCell>
         ...
         <Dialog open={!!selectedSale} onOpenChange={(open) => !open && setSelectedSale(null)}>
            <SaleDetailsDialog
               sale={selectedSale}
               customerName={selectedSale?.customerId ? customers.get(selectedSale.customerId)?.name : undefined}
         ...
         {saleToPrint && (
            <PrintInvoiceDialog
               sale={saleToPrint}
               customer={saleToPrint?.customerId ? customers.get(saleToPrint.customerId) : undefined}
