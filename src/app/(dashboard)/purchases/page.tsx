...
import { Truck, Eye, Printer, PlusCircle, CheckCircle, XCircle, AlertCircle as AlertCircleIcon, Trash2, Coins, CreditCard, Landmark, Pencil } from 'lucide-react'; // Added payment method icons, Pencil
...
import { getPurchases, getSuppliers, getProductNameById, getProductById, deletePurchase, getWarehouses } from '@/lib/data'; // Import data fetching functions, add deletePurchase, getWarehouses
...
// --- Shared payment helpers (used by both PurchaseDetailsDialog and PurchasesPage) ---
        ...
        printWindow.document.write(`
                 ...
                 <body>
                     ...
                     <div class="totals">
                        ...
                        <div><span>المبلغ المدفوع:</span> ${amountPaidNum.toFixed(2)} ج.م</div>
                        <div><span>المبلغ المتبقي:</span> ${(totalAmountNum - amountPaidNum).toFixed(2)} ج.م</div>
                        <div><strong>إجمالي الفاتورة:</strong> <strong>${totalAmountNum.toFixed(2)} ج.م</strong></div>
                     </div>
    ...
    <DialogContent className="sm:max-w-2xl"> {/* Wider dialog */}
      ...
      <div className="py-4 space-y-4">
                ...
                {detailedItems.map((item, index) => {
                  ...
                  return (
                    <TableRow key={`${item.productId}-${index}`}>
                      ...
                      <TableCell>{(itemQuantity * itemCost).toFixed(2)} ج.م</TableCell>
                    </TableRow>
                  );
                })}
              </TableBody>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-right text-base mt-4">
          ...
          <span className="font-semibold">{amountPaidNum.toFixed(2)} ج.م</span>
          <span>المبلغ المتبقي:</span>
          <span className="font-semibold">{(totalAmountNum - amountPaidNum).toFixed(2)} ج.م</span>
          <span className="text-lg font-bold col-start-1">إجمالي الفاتورة:</span>
          <span className="text-lg font-bold">{totalAmountNum.toFixed(2)} ج.م</span>
        </div>
      </div>
...
export default function PurchasesPage() {
  ...
  const { toast } = useToast(); // Keep toast as it's used in handleDeletePurchase and fetchData
  ...
  }, [purchases, searchTerm, suppliersMap]);
      ...
      <Dialog onOpenChange={(open) => !open && setSelectedPurchase(null)}>
        <div className="p-4 md:p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/purchases/add" passHref>
              <Button disabled={isLoadingWarehouses || allSuppliers.length === 0}>
                    ...
                    return (
                      <TableRow key={purchase.id}>
                        ...
                        <TableCell className="text-right space-x-1">
                          ...
                          <Link href={`/purchases/edit?id=${purchase.id}`} passHref>
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-yellow-600 hover:bg-yellow-100">
